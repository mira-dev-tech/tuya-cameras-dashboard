...
function clickExactText(doc, text) {
  ...
  const match = nodes.find((el) => {
    const t = (el.textContent || '').trim()
    return t === text && el.children.length === 0
  })
  if (match) {
    match.click()
    return true
...
async function connect() {
  ...
  if (!ok) {
    setStatus('Player indisponível')
    return
  }

  setStatus('Preparando árvore…')
  await sleep(8000)

  const doc = engineDoc()
  if (!doc) {
    setStatus('Erro no player')
    return
  }

  if (homeName) {
    setStatus(`Casa: ${homeName}`)
  ...
  setStatus(`Abrindo ${deviceName || devId}…`)
  if (!clickCameraInTree(engineDoc(), deviceName)) {
    expandDeviceTree(engineDoc())
    await sleep(800)
    if (!clickCameraInTree(engineDoc(), deviceName)) {
      setStatus('Câmera não encontrada na árvore')
      return
    }
  }
  ...
  while (!gotVideo && attempts < 4) {
    attempts += 1
    setStatus(`Reconectando (${attempts}/4)…`)
  ...
  if (!gotVideo) {
    setStatus('Sem imagem — câmera lenta ou offline')
